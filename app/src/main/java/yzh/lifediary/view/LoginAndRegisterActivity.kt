        ...
        login_btn_login.setOnClickListener {
           ...
           OkHttpClient.getOkHttpCline().newCall(
                Request.Builder().baseUrl(Constant.BASE_URL).url("login").saveCookie()
                    .post(
                        RequestBody.Builder().addParam("account", login_et_id.text.toString())
                            .addParam("password", login_et_pw.text.toString()).build()
                    ).build()
                ...
                override fun onResponse(call: Call?, response: Response?) {
                    ...
                    if (userResponse.message == "登录成功") {
                        Constant.user=userResponse.data
                        TaskExecutor.writeUserToFile(this@LoginAndRegisterActivity,userResponse.data)
                        startActivity(
                            Intent(
                                this@LoginAndRegisterActivity,
                                MainActivity::class.java
